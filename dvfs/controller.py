...
class DVFSController:
    """Apply DVFS using run_geopm_apply_ssh.sh and per-host config files."""
    ...
    def _build_apply_command(
        ...
        *,
        ...
        use_service_default = (
            self.conf_dir.resolve() == self._DEFAULT_CONF_DIR.resolve()
        ) and not self.force_direct
        ...
        *,
        ...
        if not readback_values:
            fields["verify_status"] = (
                "FAIL" if readback_reason == "target_ids_not_found_in_readback" else "NO_READBACK"
            )
