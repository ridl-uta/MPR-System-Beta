...
usage() {
  cat <<'EOF'
Usage: test_local_geopm_apply.sh [options]

Install the GEOPM apply helper on a target node, apply a test frequency to a
small core set, print readback, then restore max frequency.
...
if (( TARGET_IS_LOCAL == 0 )); then
  ...
  REMOTE_TEST_CONF="${REMOTE_TMPDIR}/test.conf"
  REMOTE_RESTORE_CONF="${REMOTE_TMPDIR}/restore.conf"
  copy_to_target "${LOCAL_TEST_CONF}" "${REMOTE_TEST_CONF}"
  copy_to_target "${LOCAL_RESTORE_CONF}" "${REMOTE_RESTORE_CONF}"
fi
...
if (( SKIP_INSTALL == 0 )); then
  ...
  if (( TARGET_IS_LOCAL == 1 )); then
    ...
    run_target_cmd "cd '${REMOTE_REPO_DIR}' && sudo bash dvfs/install_geopm_apply_systemd.sh"
  fi
fi
...
if (( TARGET_IS_LOCAL == 0 )); then
  TARGET_CONF_PATH="${REMOTE_TEST_CONF}"
  RESTORE_CONF_PATH="${REMOTE_RESTORE_CONF}"
fi
